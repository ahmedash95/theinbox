...
fn gmail_count_filtered_emails(
    ...
    state
        .storage
        .count_filtered_emails(&email, &filter_ids, unread_only)
}

#[tauri::command]
fn gmail_filter_match_counts(
    state: State<AppState>,
    email: String,
    unread_only: bool,
) -> Result<Vec<FilterMatchCount>, String> {
    let counts = state.storage.filter_match_counts(&email, unread_only)?;
    Ok(counts
        .into_iter()
        .map(|(id, count)| FilterMatchCount { id, count })
        .collect())
}
...
/// Mark Gmail emails as unread (batch operation)
#[tauri::command]
async fn gmail_mark_as_unread(
    state: State<'_, AppState>,
    email: String,
    uids: Vec<u32>,
) -> Result<usize, String> {
...
#[tauri::command]
async fn gmail_refresh_filtered_emails(
    app: AppHandle,
    state: State<'_, AppState>,
    email: String,
    force_full: bool,
