        ...
        >
          ...
          <button class="flex min-w-0 flex-1 flex-col gap-1 text-left" @click="emit('view-email', email)">
            <div class="flex items-center justify-between gap-3">
              ...
              <div class="flex items-center gap-2">
                <div
                  v-if="email.matchingFilters.length > 0"
                  class="flex max-w-[140px] flex-nowrap items-center justify-end gap-1 overflow-hidden"
                >
                  <Badge v-for="tag in email.matchingFilters" :key="tag" variant="secondary" class="shrink-0">
                    {{ tag }}
                  </Badge>
                </div>
